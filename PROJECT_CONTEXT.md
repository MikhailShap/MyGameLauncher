...
11. **Плеер трейлера — НАТИВНАЯ панель media_kit** (`show_controls=True`), не
    кастомная Flet-панель (§7 устарел). Кнопка/клик трейлера в wishlist зовёт
    ...
    ⚠️ Оставшийся хвост: `_trailer_reveal`/`_video_fullscreen` не выставляются —
    проверить возврат окна из нативного fullscreen при закрытии плеера.
