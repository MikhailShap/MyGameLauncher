...
> сделано и над чем работаем. Версия на момент написания — **v1.9.12**.
...
| `wishlist_manager.py` | раздел «Желаемое»: `WishlistItem`, `WishlistManager`, Steam Store API (`steam_search`, `steam_appdetails`, `steam_extra_info`, `fetch_game_details`), парсинг HLS-трейлеров, кэш деталей. |
| `steam_owned.py` | раздел «Steam: не стоят»: `OwnedGame`, `SteamOwnedManager`, список покупок аккаунта (Web API `GetOwnedGames`), сверка с `appmanifest_*.acf`, кэш обложек. |
...
│  ├─ steam_owned.json      # купленные в Steam игры (кэш ответа Web API)
│  └─ settings.json         # настройки (темы, excluded_paths, api_keys, ...)
├─ cache\
│  ├─ icons\<hash>.jpg       # обложки
│  ├─ heroes\<uid>[_<ts>].jpg# landscape-арт для BigPicture/деталей
│  ├─ wishlist_headers\<appid>.jpg   # обложки карточек желаемого
│  ├─ steam_headers\<appid>.jpg      # обложки карточек «Steam: не стоят»
...
> ⚠️ **Из скриптов/CI запускать `venv\Scripts\python.exe Build.py`, а не
> `python Build.py`.** На Windows `os.execv` (перезапуск через venv в начале
> Build.py) процесс НЕ подменяет: он порождает дочерний и тут же завершает
> текущий с кодом 0. Родительская оболочка считает сборку успешной и убивает
> дерево процессов на середине анализа PyInstaller — в `dist\` остаётся старый
> exe. В интерактивном терминале не проявляется (никто не убивает потомка).
...
- **Раздел «Желаемое» (Steam wishlist):**
  ...
  - **поиск по своему списку** (название/студия/жанр/appid, debounce 250 мс);
  - **бейдж «ДЕМО»** на карточке, если у игры есть демоверсия в Steam
    (+ фильтр «Есть демоверсия» и кнопка «Установить демо» в деталке);
  - **оценка игроков** бейджем на обложке (процент + цвет по шкале Steam,
    в подсказке — словесная оценка и число отзывов); клик по бейджу/чипу и
    кнопка «Отзывы» в деталке открывают отзывы в Steam;
...
- **Раздел «Steam: не стоят»** — купленные в аккаунте игры, которых нет на
  диске: сверка списка покупок с `appmanifest_*.acf`, поиск, сортировка (в т.ч.
  «ни разу не запускавшиеся»), кнопка «Установить» (`steam://install/<appid>`),
  «убрать из списка». Требует бесплатный Steam Web API-ключ (см. грабля 13).
...
3d. **«Показать ещё» обязано ДОПИСЫВАТЬ, а не пересобирать.** Прокрутка живёт
   в самом скроллируемом контроле: пересоздали `ft.Column(scroll=...)` —
   получили новый ScrollController с нулевым смещением, и список прыгает в
   начало (жалоба 2026-07-28). Поэтому ряд карточек и кнопка «Показать ещё»
   держатся в ПОСТОЯННЫХ контролах (`_wl_cards_row` / `_wl_more_holder`,
   `_so_cards_row` / `_so_more_holder`), а пагинация делает
   `row.controls.extend(новые)` + подмену `holder.content` и обновляет только
   их. Полная пересборка тела остаётся только для смены фильтра/поиска, где
   возврат к началу списка как раз уместен.

3c. **Большой СЕРЫЙ прямоугольник в UI = сбой раскладки Flutter**, а не пустой
   контейнер и не «не загрузилась картинка». В release-сборке упавший виджет
   заменяется на ErrorWidget — плоскую серую заливку на всё доступное место,
   без текста ошибки (в логах Python при этом ТИШИНА, там ошибки нет).
   Конкретный случай (2026-07-27): `ft.Row(wrap=True)` с распоркой
   `ft.Container(expand=True)` внутри. `wrap=True` — это Flutter `Wrap`, а
   `expand` — `Expanded`, который допустим только внутри Flex (Row/Column);
   в Wrap он падает. Правило: **в Row с `wrap=True` не должно быть детей с
   `expand`** — либо убрать wrap, либо заменить распорку фиксированным
   `Container(width=...)`. Проверить весь файл разом:
   `grep`-ом не выйдет (многострочные вызовы) — обходить AST, ища `Row` с
   `wrap=True`, в исходнике `controls` которого встречается `expand=True`.
...
11. **Плеер трейлера — НАТИВНАЯ панель media_kit** (`show_controls=True`), не
    ...
    - **Буфер обмена — НАТИВНЫЙ Win32, не Flet** (`_win_clipboard_set`,
      CF_UNICODETEXT через ctypes). Буфер Flet 0.80 — асинхронный СЕРВИС
      (`page.clipboard.set`, старого `page.set_clipboard()` нет), и в этом
      беда: значение уезжает в Flutter-клиент через канал сообщений, а ошибка
      всплывает в done-callback задачи `run_task` и **молча теряется** — в логе
      тишина, но снекбар «Скопировано» при этом рисовался. Плюс каждое
      обращение к `page.clipboard` (property помечен deprecated) создаёт и
      РЕГИСТРИРУЕТ новый сервис — список `page._services` рос на каждый клик.
      Нативная запись синхронная: вернула True — текст точно в буфере.
      Сервис Flet оставлен фоллбеком (не-Windows / отказ Win32), снекбар
      успеха рисуется только по факту записи. Каждое копирование пишет строку
      в лог — чтобы «не копируется» отличалось от «клик не доходит».
    ...
    - **Сторож зависания (`_trailer_stall_watchdog`) обязан жить «поколениями».**
      Он ловит случай «поток не отдал ошибку, но и не играет» и через 8с
      уводит на HLS. Грабля 2026-07-27: сторож держал ССЫЛКУ на плеер, под
      который был заведён. Смена качества пересоздаёт плеер (media_kit не
      меняет источник на лету), старый останавливается — и сторож спрашивал
      позицию у мёртвого плеера, получал 0, считал это зависанием и насильно
      переоткрывал трейлер поверх нормально играющего. Симптом: выбрал
      качество → fullscreen → перемотка → чёрный экран 00:00/00:00 ровно через
      8с после ОТКРЫТИЯ плеера. Лечение: счётчик `_trailer_watchdog_token`
      (смена качества его увеличивает — старый сторож молчит), плеер берётся
      ТЕКУЩИЙ из `_trailer_sizing`, а не захваченный.
    - **Нулевая позиция ≠ зависание.** Пользователь мог нажать паузу или
      перемотать в начало. Сторож спрашивает `is_playing()` и берёт вторую
      пробу позиции через 1.5с. И **в нативном fullscreen откат не делается
      вовсе** — пересоздание оверлея под полноэкранной поверхностью media_kit
      как раз и даёт мёртвый чёрный экран.
...
12. **Патчи Flet 0.80 — `flet_patches.py`** (заведён 2026-07-24 после
    краш-экрана в лаунчере при вылете игры, подробности —
    `CRASH_MEMORYERROR_ANALYSIS.md`). Применяются в `main.py` сразу после
    импортов flet/flet_video:
    - **дешёвый `__repr__` у контролов.** `DiffBuilder._compare_lists`
      (`object_patch.py`) начинается с `logger.debug(f"... {src} {dst}")` —
      f-string считается ВСЕГДА, а dataclass-repr контрола рекурсивно печатает
      всё поддерево (замер: 472 карточки = 3.4 МБ и 31 мс на КАЖДЫЙ
      `page.update()`, причём списки реприт**ся** дважды). После патча — 8 КБ /
      0.2 мс. repr в Flet только диагностический, на протокол не влияет.
    - **memory-guard.** Flet после каждого события сам зовёт `page.update()`;
      если в этот момент в системе нет памяти (сразу после вылета тяжёлой
      игры — Windows пишет в System-лог событие 26 «Слишком мало виртуальной
      памяти»), `MemoryError` уходит в `Session.error` → клиент рисует
      SESSION_CRASHED поверх лаунчера. Теперь `after_event` глотает
      `MemoryError`, а `Session.error` не шлёт краш-экран именно для него
      (остальные ошибки доходят как раньше).
    - Новый локальный модуль → добавлен в `Build.py` (add_data +
      hidden_imports) и `CyberLauncher.spec`.

13. **Список покупок Steam отдаётся ТОЛЬКО по Web API-ключу** (проверено
    2026-07-26). Прежний бесключевой путь — XML профиля
    `steamcommunity.com/profiles/<id>/games?tab=all&xml=1` — Valve закрыла:
    анонимному запросу приходит HTML страницы входа, причём даже для профиля с
    `privacyState=public`. Поэтому `steam_owned.py` работает через
    `IPlayerService/GetOwnedGames` (ключ бесплатный,
    https://steamcommunity.com/dev/apikey), ключ лежит в
    `settings.json → api_keys.steam`, SteamID — в `settings.json → steam_id64`
    (подставляется из `find_local_steamid64()`). XML оставлен фоллбеком: при
    HTML-ответе тихо сдаётся, UI просит завести ключ. Пустой ответ при валидном
    ключе = закрыты «Игровые подробности» в приватности Steam — сообщение
    должно говорить именно про это, а не «Steam недоступен».
    - «Установлено или нет» считаем по `appmanifest_<appid>.acf`, а не по папке
      в `common` (Steam оставляет огрызки папок — та же логика, что у
      `GameManager.steam_install_present`).

14. **Поле ввода нельзя пересоздавать на каждый ребилд вью.** Поиск в
    «Желаемом» (`_wl_search_field`) и в «Steam: не стоят» (`_so_search_field`)
    создаётся ОДИН РАЗ и переиспользуется: `build_wishlist_view()` вызывается
    после каждого чиха (приоритет, фильтр, ресайз), и новый `ft.TextField` на
    каждом нажатии клавиши убивал бы фокус и текст. По той же причине сам поиск
    обновляет НЕ всю вью, а держатели `_wl_body_holder` / `_wl_filters_holder`
    (`_wl_refresh_body`, `_so_refresh_body`) — точечным `control.update()`.

17. **Клик по всплывающему уведомлению трея pystray НЕ ловит.** Её
    `Icon._on_notify` реагирует только на `WM_LBUTTONUP`, то есть на клик по
    ИКОНКЕ; Windows же присылает клик по самому тосту отдельным кодом
    `NIN_BALLOONUSERCLICK` (WM_USER+5 = 1029), и он просто игнорировался —
    нажатие на уведомление о релизе молча ничего не делало (жалоба
    2026-07-29). Лечится точечным патчем `_patch_pystray_balloon_click()` в
    `main.py` (идемпотентен, применяется в `create_tray_icon`); что делать по
    клику, кладётся в `TRAY_BALLOON_CLICK["fn"]` тем, кто показывает
    уведомление. Колбэк прилетает из ПОТОКА pystray, поэтому работа с UI —
    только через `page.run_task` (потокобезопасен), а поднятие окна — через
    `TRAY_SHOW_WINDOW["fn"]` (то же, что пункт «Показать» в меню трея).
...
16. **Оценки игроков в appdetails НЕТ** — там только `recommendations.total`
    ...
    (ключ не нужен, `l=russian` локализует подпись, `query_summary` даёт
    `review_score_desc` / `total_positive` / `total_reviews`). Это ДРУГОЙ
    эндпоинт, под лимит appdetails он не попадает, поэтому в
    `fill_missing_extras` запрос идёт в той же итерации — темп прохода задаёт
    пауза, а не число запросов. У невышедших игр Steam отвечает success=1 с
    нулями: это валидное «отзывов пока нет», а не ошибка (поле
    `reviews_checked` отличает его от «ещё не спрашивали»). Рейтинг заодно
    освежается при сетевом обновлении деталей (кэш 7 дней).

15. **Демоверсия в Steam — отдельное приложение** со своим appid: appdetails
    отдаёт её в `demos: [{appid, description}]`, а не флагом. Признак и
    категории добираются ОДНИМ лёгким запросом `filters=categories,demos`
    (`steam_extra_info`) — лимит appdetails общий (~200 запросов / 5 мин), два
    прохода вместо одного упирались бы в троттлинг. Фоновая добивка
    (`fill_missing_extras`, поле `demo_checked` отличает «демо нет» от «ещё не
    спрашивали») стартует один раз за сессию при заходе в раздел и не
    конкурирует с `fill_missing_details`.
