...
11. **Плеер трейлера — НАТИВНАЯ панель media_kit** (`show_controls=True`), не
    ...
    - **Буфер обмена в Flet 0.80 — СЕРВИС с async-методом**:
      `page.run_task(page.clipboard.set, text)`. Старого `page.set_clipboard()`
      НЕТ (вызов молча падал бы в except).
    - **Обложки карточек желаемого кэшируются на диск**
      (`cache/wishlist_headers/<appid>.jpg`, `header_image_src()`): Flutter
      держит картинки только в памяти сессии, поэтому 450+ обложек качались с
      CDN при КАЖДОМ запуске — это и была «медленность UI». Раздел рисуется
      страницами по 24 (`_WISHLIST_PAGE_SIZE`) + кэш готовых карточек.
