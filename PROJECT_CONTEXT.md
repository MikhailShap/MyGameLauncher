...
### Кастомный видео-плеер трейлеров (в разделе «Желаемое») — основной фокус
...
и пишем **свою** на чистом Flet. Сделано: play/pause, перемотка (slider +
опрос позиции), громкость, выбор качества (HLS-варианты), in-app максимайз,
спиннер загрузки, отключён HW-декод против фризов, кнопка «В браузере» как
fallback.

**Что ещё шероховато / надо доделать:**
- стабильность воспроизведения HLS Steam с akamai (бывают сетевые ошибки
  `ffurl_read`, `Failed to recognize file format`);
- переключение качества (file://-плейлист с раздельным аудио) — проверять
  надёжность;
- возможно: сохранение позиции при смене качества; более плавная шкала;
...
3. **Контролы поверх media_kit-видео не получают события** и обновление
   вложенных в `page.overlay` контролов через `page.update()` ненадёжно —
   обновлять **конкретный контрол** (`control.update()`), а лучше его
   контейнер. Все методы `flet_video` (`play`, `seek`, `get_*`) — **async**,
   звать через `page.run_task`.
